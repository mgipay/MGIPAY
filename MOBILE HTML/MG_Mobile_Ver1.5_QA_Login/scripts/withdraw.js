...
function currencyfocusout()
{
	...
	if($('#receiveAmount').val().trim() == "" || $('#receiveAmount').val() == "")
	{
		$("#amountError").show();
		...
		/*if(feeResponse == "" || feeResponse == null || feeResponse == undefined)
		{*/
		...
		/*}else
		{
			alert("else" +feeResponse);
			var feeValues;
			if($.browser.chrome){
				feeValues = localStorage.getItem("feeResponse");
			}
			else {
				feeValues = $.cookie("feeResponse");
			}
			feeResponse = JSON.parse(feeValues);
			alert("parse" +feeResponse);
			feeSuccessHandler(feeResponse);
		}
*/	}  
}
...
function reviewTransaction()
{
	...
	if(isNaN($("#receiveAmount").val()) || $("#receiveAmount").val() == "" || $('#receiveAmount').val().trim() == ""){
		$("#amountError").html("<sup>*</sup>"+resources.validAmt);
		$("#amountError").show();
		return false;
	}
...
var commitSuccessHandler = function(response){
	...
	if(response.transactionSuccess == true)
	{
		...
		pageDetails.push($("#receiveAmount").val(), ddValue, feeAmt, totalAmt);
		if($.browser.chrome){
