...
var withdrawSuccessHandler = function(response){
	alert("withdraw"+JSON.stringify(response));//referenceNumber":"15943292","transactionSuccess":true}
	$("#preview").hide();
	if(response.transactionSuccess == true)
	{
		...
		if($.browser.chrome)
			localStorage.setItem("confirmationResp", response); 
		else 
			$.cookie("confirmationResp", response); 
